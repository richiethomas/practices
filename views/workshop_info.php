...
if ($wk['location_id'] == ONLINE_LOCATION_ID) {
	$where_html = "		<tr><td scope=\"row\"><span class='oi oi-map title='map' aria-hidden='true'></span> Where:</td><td>{$wk['place']}<br>Link for online class: {$wk['online_url']}</tr>";
} else {
	$where_html = "		<tr><td scope=\"row\"><span class='oi oi-map title='map' aria-hidden='true'></span> Where:</td><td>{$wk['place']} {$wk['lwhere']}</tr>";
}
