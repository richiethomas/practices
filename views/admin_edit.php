...
echo "<div class='row mt-md-3 admin-edit-workshop'>\n";
		...
		foreach ($statuses as $stid => $status_name) {
			...
			foreach ($lists[$stid] as $s) {
				echo "<div class='row my-3'><div class='col-md-6'>".Wbhkit\checkbox('users', $s['id'], "<a href='admin_user.php?guest_id={$s['id']}&wid={$wk['id']}'>{$s['nice_name']}</a> <small>".date('M j g:ia', strtotime($s['last_modified']))."</small>", $s['paid'], true)."</div>".
				"<div class='col-md-6'>
